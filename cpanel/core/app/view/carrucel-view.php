    ...
    <div class="col-lg-12">
        <div class="card" id="leadsList">
            <div class="card-header bg-warning d-flex justify-content-between align-items-center py-2">
                <h6 class="card-title text-white mb-0">Gestion de Carrucel</h6>
...
<div class="modal fade" id="showModal" tabindex="-1" aria-labelledby="userModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-light p-2">
                <h5 class="modal-title" id="userModalLabel">Nuevo Carrucel</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form method="POST" class="user-form" autocomplete="off" action="index.php?action=registro">
                <div class="modal-body">
                    <div class="row g-2">
                        ...
                        <div class="col-lg-12">
                            <label for="apellido" class="form-label">Imagen</label>
                            <input type="file" id="imagen" name="imagen" class="form-control form-control-sm" required 
                                />
                        </div>

                        <div class="col-lg-12">
                            <label for="enlace" class="form-label">Enlace</label>
                            <input type="url" id="enlace" name="enlace" class="form-control form-control-sm" 
                                   />
                        </div>
...
<script>
    $(document).ready(function(){
        $('#mitabla').DataTable({
            ...
            "processing": true,
            "serverSide": true,
            "ajax": "index.php?action=usuario&actions=1",
            "columns": [
                { "data": "dni" },
                { "data": "nombre" },
                { "data": "email" },
        ...
        $('#mitabla').on('click', '.edit-btn', function(){
            var id = $(this).data('id');
            $.ajax({
                url: 'index.php?action=usuario&actions=2',
                type: 'POST',
                data: {id: id},
                dataType: 'json',
